...
import { DocumentTextIcon, ChatBubbleBottomCenterTextIcon, ClockIcon, ArrowUpOnSquareIcon } from '@/components/Icons';
import { Share2Icon } from '@/components/Icons/radixIcons';
import { Divider, Button, Drawer } from '@mantine/core'
import OnlineContributors from '@/components/OnlineContributors';
...
const Header = () => {
    ...
    return (
        <header className='flex border-b fixed z-2 top-0 w-full shadow-lg border-gray-100 py-5 px-4 sm:px-6 bg-white justify-between h-[64px]'>
            ...
            <div className='flex gap-3 items-center'>
                <OnlineContributors className='hidden sm:flex' />
                <Divider orientation="vertical" className='hidden sm:flex w-[0.01rem] bg-zinc-300' />

                <ChatBubbleBottomCenterTextIcon className='w-5 h-5 cursor-pointer text-gray-500' />
                <ClockIcon className='w-5 h-5 cursor-pointer text-gray-500' />
                <Divider orientation="vertical" className='hidden sm:flex w-[0.01rem] bg-zinc-300' />
